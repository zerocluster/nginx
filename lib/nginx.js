...
export default class Nginx extends Events {
    ...
    async run () {
        ...
        this.#docker
            .on( "add", data => this.addService( data.name, { "hostname": data.hostname, "options": data.options } ) )
        ...
        for ( const service of services ) {
            await this.addService( service.name, { "hostname": service.hostname, "options": service.options } );
        }
    ...
    async addService ( id, { hostname, options } = {} ) {
        this.#services[id] ??= new NginxService( this, id, { hostname } );
    ...
    validateNginxSizeValue ( value ) {
        return /^\d+[kKmMgG]?$/.test( value + "" );
    }

    validateNginxTimeValue ( value ) {
        value = value + "";
