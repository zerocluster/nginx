...
export default class NginxService {
    ...
    async #addPeer ( peer ) {
        ...
        if ( this.#isHttpEnabled ) {
            ...
            if ( res.ok ) {
                console.log( `Service: ${this.name}, http upstream added: ${peer}:80` );

                this.#peers.add( peer );
            }
            else {
                console.log( `Service: ${this.name}, failed to add http upstream: ${peer}:80` );
            }
        }
        ...
        if ( this.#isStreamEnabled ) {
            ...
            if ( res.ok ) {
                console.log( `Service: ${this.name}, stream upstream added: ${peer}:${this.#streamPort}` );

                this.#peers.add( peer );
            }
            else {
                console.log( `Service: ${this.name}, failed to add stream upstream: ${peer}:${this.#streamPort}` );
            }
        }
    }
    ...
    async #removePeer ( peer ) {
        ...
        if ( this.#isHttpEnabled ) {
            const res = await fetch( `http://127.0.0.1/dynamic-upstream?upstream=http-${this.id}&remove=&server=${peer}` );

            if ( res.ok ) {
                console.log( `Service: ${this.name}, http upstream removed: ${peer}:80` );
            }
            else {
                console.log( `Service: ${this.name}, failed to remove http upstream: ${peer}:80` );
            }
        }

        if ( this.#isStreamEnabled ) {
            const res = await fetch( `http://127.0.0.1/dynamic-upstream?upstream=stream-${this.id}&remove=&server=${peer}:${this.#streamPort}&stream=` );

            if ( res.ok ) {
                console.log( `Service: ${this.name}, stream upstream removed: ${peer}:${this.#streamPort}` );
            }
            else {
                console.log( `Service: ${this.name}, failed to remove stream upstream: ${peer}:${this.#streamPort}` );
            }
        }
