...
export default class NginxService {
    ...
    update ( options = {} ) {
        ...
        if ( !this.#nginx.validateNginxSizeValue( options.httpMaxBodySize ) ) options.httpMaxBodySize = DEFAULT_OPTIONS.httpMaxBodySize;
        ...
        console.log( `Service: ${this.name}, updated:`, JSON.stringify( options, null, 4 ) );
    ...
    async #addUpstream ( upstream ) {
        ...
        if ( this.#isStreamEnabled ) {
            for ( const port of this.#streamPort ) {
                ...
                if ( res.ok ) {
                    console.log( `Service: ${this.name}, stream upstream added: ${upstream}:${port}` );
                }
                else {
                    console.log( `Service: ${this.name}, failed to add stream upstream: ${upstream}:${port}` );
                }
            }
        }
    }
    ...
    async #removeUpstream ( upstream ) {
        ...
        if ( this.#isStreamEnabled ) {
            for ( const port of this.#streamPort ) {
                ...
                if ( res.ok ) {
                    console.log( `Service: ${this.name}, stream upstream removed: ${upstream}:${port}` );
                }
                else {
                    console.log( `Service: ${this.name}, failed to remove stream upstream: ${upstream}:${port}` );
                }
            }
        }
    }
