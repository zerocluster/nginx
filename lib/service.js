...
export default class NginxService {
    ...
    update ( options = {} ) {
        ...
        console.log( `Service: ${this.name}, updated:\n`, JSON.stringify( options, null, 4 ) );
    ...
    async #addUpstream ( upstream ) {
        ...
        if ( this.#isHttpEnabled ) {
            ...
            if ( res.ok ) {
                console.log( `Service: ${this.name}, http upstream added: ${upstream}:80` );

                this.#upstreams.add( upstream );
            }
            else {
                console.log( `Service: ${this.name}, failed to add http upstream: ${upstream}:80` );
            }
        }

        if ( this.#isStreamEnabled ) {
            const res = await fetch( `http://127.0.0.1/dynamic-upstream?upstream=stream-${this.id}&add=&server=${upstream}:${this.#streamPort}&stream=` );

            if ( res.ok ) {
                console.log( `Service: ${this.name}, stream upstream added: ${upstream}:${this.#streamPort}` );

                this.#upstreams.add( upstream );
            }
            else {
                console.log( `Service: ${this.name}, failed to add stream upstream: ${upstream}:${this.#streamPort}` );
            }
        }
    }
    ...
    async #removeUpstream ( upstream ) {
        ...
        if ( this.#isHttpEnabled ) {
            ...
            if ( res.ok ) {
                console.log( `Service: ${this.name}, http upstream removed: ${upstream}:80` );
            }
            else {
                console.log( `Service: ${this.name}, failed to remove http upstream: ${upstream}:80` );
            }
        }

        if ( this.#isStreamEnabled ) {
            const res = await fetch( `http://127.0.0.1/dynamic-upstream?upstream=stream-${this.id}&remove=&server=${upstream}:${this.#streamPort}&stream=` );

            if ( res.ok ) {
                console.log( `Service: ${this.name}, stream upstream removed: ${upstream}:${this.#streamPort}` );
            }
            else {
                console.log( `Service: ${this.name}, failed to remove stream upstream: ${upstream}:${this.#streamPort}` );
            }
        }
