...
import { validateNginxSizeValue, validateNginxDurationValue } from "#lib/validator";
...
const optionsValidator = ajv.compile( readConfig( "#resources/schemas/service-options.schema.yaml", { "resolve": import.meta.url } ) );
    ...
    constructor ( nginx, id, name, { hostname } = {} ) {
        ...
        if ( this.#hostname ) {
            this.#dnsWatcher = new DnsWatcher( this.#hostname, {
                "family": this.#nginx.listenIpFamily,
                "minInterval": 1000,
                "maxInterval": 60000,
                "step": 5000,
            } )
    ...
    update ( options = {} ) {
        ...
        if ( this.#isHttpEnabled ) {
            const conf = ejs.render( fs.readFileSync( resolve( "#resources/templates/vhost.http.nginx.conf", import.meta.url ), "utf8" ), {
                "id": this.id,
                "listen_ip_family": this.#nginx.listenIpFamily,
                "proxy_protocol": this.#nginx.proxyProtocol,
