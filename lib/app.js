...
class Service {
    ...
    async #addPeer ( peer ) {
        ...
        if ( this.#labels["nginx.server-name"] ) {
            const res = await fetch( `http://127.0.0.1/dynamic-upstream?upstream=http-${this.id}&add=&server=${peer}:80` );

            if ( res.ok ) {
                console.log( `Service: ${this.name}, http peer added: ${peer}` );

                this.#peers.add( peer );
            }
            else {
                console.log( `Service: ${this.name}, failed to add http peer: ${peer}` );
            }
        }

        if ( this.#labels["nginx.stream-port"] ) {
            const res = await fetch( `http://127.0.0.1/dynamic-upstream?upstream=stream-${this.id}&add=&server=${peer}:${this.#labels["nginx.stream-port"]}` );
            ...
            if ( res.ok ) {
                ...
                console.log( "---------------------------------" );
                console.log( res + "" );
                console.log( `http://127.0.0.1/dynamic-upstream?upstream=http-${this.id}&add=&server=${peer}:80` );
                console.log( `http://127.0.0.1/dynamic-upstream?upstream=stream-${this.id}&add=&server=${peer}:${this.#labels["nginx.stream-port"]}` );
