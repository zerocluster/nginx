...
class Service {
    ...
    async updatePeers () {
        const oldPeers = await this.#getPeers(),
            newPeers = await this.#resolvePeers();

        var updated;

        // check added peers
        for ( const peer of newPeers ) if ( !oldPeers.has( peer ) ) updated = true;

        // check removed peers
        for ( const peer of oldPeers ) if ( !newPeers.has( peer ) ) updated = true;

        if ( updated ) {
            this.#peers = newPeers;

            await this.updateVhost();
        }
    ...
    removeVhost () {
        fs.rmSync( VHOSTS_DIR + "/" + this.id + ".nginx.conf", { "force": true } );
    }
...
module.exports = class Nginx {
    ...
    async #updateService ( service ) {
        const nginxServerName = service.nginxServerName;
        ...
        // service alreay exists
            ...
            // label was updated
            if ( this.#services[service.id].nginxServerName !== nginxServerName ) {
    ...
    async #updateUpstreams () {
        while ( 1 ) {
            ...
            for ( const service of Object.values( this.#services ) ) {
                if ( await service.updatePeers() ) reload = true;
            }
