...
class Service {
    ...
    update ( labels ) {
        ...
        // generate vhost conf
        const conf = ejs.render( fs.readFileSync( require.resolve( "#resources/templates/vhost.nginx.conf" ), "utf8" ), {
            "id": this.id,
            "cache_dir": CACHE_DIR,
...
export default class Nginx extends Events {
    ...
    async run () {
        ...
        const conf = ejs.render( fs.readFileSync( require.resolve( "#resources/templates/nginx.conf" ), "utf8" ), {
            "base_dir": BASE_DIR,
