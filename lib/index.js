    ...
    #startSwarmListener () {
        this.#docker.getEvents( { "filters": JSON.stringify( { "scope": ["swarm"], "type": ["service"] } ) }, ( err, stream ) => {
            ...
            this.#docker.modem.followProgress( stream,
                () => this.#startSwarmListener(),
                async data => {
                    if ( data.Action === "create" ) this.#addVhost( data.Actor.ID );
                    else if ( data.Action === "remove" ) this.#removeVhost( data.Actor.ID );
                    else if ( data.Action === "update" ) this.#updateVhost( data.Actor.ID );
                } );
        } );
    }

    async #updateVhost ( id ) {}
