...
const NGINX_STARTUP_DELAY = 5000;
        ...
        if ( labels ) {
            ...
            for ( const label in LABELS ) {
                ...
                if ( this.#labels[label] !== labels[label] ) {
                    ...
                    console.log( `Service: ${this.name}, label: ${label} = ${this.#labels[label]}` );
                }
            }
...
module.exports = class Nginx {
    ...
    async reloadConfig ( delay ) {
        ...
        if ( ok ) this.#proc.kill( "SIGHUP" );

        if ( !this.#pendingReload ) await this.#onReload();
    ...
    async #updateService ( service ) {
        ...
        if ( this.#services[service.id] ) {
            ...
            // nginx server name label was updated
                ...
                return oldService.update( service.labels );
            }
        }
    ...
    async #onReload () {
        ...
        return this.#updateUpstreams();
    }

    async #updateUpstreamsScheduler () {
        while ( 1 ) {
    ...
    async #updateUpstreams () {
        return Promise.all( Object.values( this.#services ).map( service => service.updateUpstreams() ) );
    }
};
