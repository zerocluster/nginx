...
class Service {
    ...
    update ( labels ) {
        ...
        // generate vhost conf
        const conf = ejs.render( fs.readFileSync( resolve( "#resources/templates/vhost.nginx.conf", import.meta.url ), "utf8" ), {
            "id": this.id,
            "cache_dir": CACHE_DIR,
...
export default class Nginx extends Events {
    ...
    async #startSwarmListener () {
        ...
        const stream = await DOCKER_ENGINE.monitorSysytemEvents( { "filters": { "scope": ["swarm"], "type": ["service"] } } );

        stream.on( "event", async data => {
