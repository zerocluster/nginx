...
class Service {
    ...
    async updateVhost ( options = {} ) {
        var updated;

        if ( !this.#peers ) {
            this.#peers = await this.#resolvePeers();
            ...
            // check added peers
            for ( const peer of newPeers ) if ( !this.#peers.has( peer ) ) updated = true;

            // check removed peers
            for ( const peer of this.#peers ) if ( !newPeers.has( peer ) ) updated = true;

            if ( updated ) this.#peers = newPeers;
        }

        // service has no peers
        if ( !this.#peers.size ) return this.removeVhost();

        if ( options.nginxServerName ) {
            if ( this.#nginxServerName !== options.nginxServerName ) {
                updated = true;
        ...
        console.log( `Vhost for service "${this.name}" updated. Peers: ${[...this.#peers].join( ", " )}` );
        ...
        const conf = ejs.render( fs.readFileSync( __dirname + "/../resources/tmpl/vhost.nginx.conf", "utf8" ), {
            ...
            "server_name": this.nginxServerName,
            "cache_dir": CACHE_DIR,
            "peers": this.#peers,
        } );
    ...
    removeCache () {
        fs.rmSync( CACHE_DIR + "/" + this.id, { "recursive": true, "force": true } );
    }
}
...
module.exports = class Nginx {
    ...
    async #updateService ( service ) {
        ...
        if ( this.#services[service.id] ) {
            ...
            if ( !service.nginxServerName ) {
                return this.#removeService( service.id );
            }

            // nginx server name label was updated
            else {
                return service.updateVhost( service.nginxServerName );
            }
        }
        ...
        // new service
            ...
            if ( service.nginxServerName ) return this.#addService( service );
        }
    }
    ...
    #removeService ( serviceId ) {
        ...
        console.log( `Service removed: ${this.#services[serviceId].name}` );
    ...
    async #addService ( service ) {
        if ( !service.nginxServerName ) return this.#removeService( service.id );

        console.log( `Service added: ${service.name}` );

        this.#services[service.id] = service;

        return await service.updateVhost();
    }
    ...
    async #updateUpstreams () {
        while ( 1 ) {
            ...
            for ( const service of Object.values( this.#services ) ) {
                if ( await service.updateVhost() ) reload = true;
            }
